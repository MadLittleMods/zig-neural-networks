...
const CustomScalarLayer = @import("CustomScalarLayer.zig");
...
test "Serialize/deserialize neural network with custom layer types" {
    ...
    // for deserialization).
    try Layer.registerCustomLayer(CustomScalarLayer, allocator);
    ...
    var custom_noise_layer = try CustomScalarLayer.init(
        1.015,
    );
