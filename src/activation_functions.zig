...
///
/// It looks like LeakyReLU except with a smooth transition in the corner instead of
/// that sharp transition that asymotically approaches -1. Not having a corner in the
/// function makes some training routines more stable, and even though there are few
/// more calculations involed with ELU, the network is usually large enough that matrix
...
/// Activation functions allow a layer to have a non-linear affect on the output so
/// they can bend the boundary around the data. Without this, the network would only be
/// able to separate data with a straight line.
///
...
pub const ActivationFunction = union(enum) {
    ...
    /// to determine whether we need to use the more expensive `jacobian_row` function
    /// or an efficient shortcut with the `derivative` function.
    ///
    /// Hint: Most activation functions are single-input activation functions (except
    /// for SoftMax).
